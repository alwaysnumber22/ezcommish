...
def new_league():
    ...
    if not c: return redirect(url_for('index'))
    if request.method=='POST':
        name=request.form['name'].strip(); sport=request.form['sport']; team_count=int(request.form['team_count']); tz=request.form['timezone'].strip() or 'Pacific Time (PT)'
        if team_count<4 or team_count>32: flash('League size must be 4-32.'); return redirect(request.url)
...
def edit_managers(league_id):
    ...
    if request.method=='POST':
        name=request.form.get('name','').strip(); phone=request.form.get('phone','').strip()
        if name and phone:
            db().execute('INSERT INTO managers(league_id,name,phone) VALUES(?,?,?)',(league_id,name,phone)); db().commit()
...
def choose_dates(league_id):
    ...
    if not league: abort(404)
    if request.method=='POST':
        dates=[request.form.get(f'date{i}') for i in range(1,4)]
        deadline=request.form.get('deadline')
        if any(not d for d in dates) or len(set(dates))<3:
            flash('Choose three different dates.'); return redirect(request.url)
...
def select_round1(league_id,option_id):
    league=owned_league(league_id)
    if not league: abort(404)
    r1=get_active_round(league_id,1)
...
def select_round2(league_id,option_id):
    league=owned_league(league_id)
    if not league: abort(404)
    r2=get_active_round(league_id,2)
...
def team_name(token):
    ...
    m=db().execute('SELECT * FROM managers WHERE id=?',(mid,)).fetchone()
    if request.method=='POST':
...
def submitted(token):
    ...
    total=db().execute('SELECT COUNT(*) c FROM managers WHERE league_id=?',(league['id'],)).fetchone()['c']
    responded=0
    for m in db().execute('SELECT id FROM managers WHERE league_id=?',(league['id'],)).fetchall():
...
def calendar_ics(token):
    league=db().execute('SELECT * FROM leagues WHERE share_token=?',(token,)).fetchone()
    if not league or not league['final_date'] or not league['final_time']: abort(404)
